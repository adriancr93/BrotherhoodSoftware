		...
		if ( isset( $_POST['post_id'] ) && is_numeric( $_POST['post_id'] ) ) {
			if ( current_user_can( 'edit_posts', $_POST['post_id'] ) ) {
				if ( delete_post_meta( $_POST['post_id'], 'layout_settings' ) ) {
					$resp = array(
						'success' => true,
						'message' => esc_html__( 'Data was removed. Please, update the page.', 'wagency' ),
					);
				} else {
					$resp = array(
						'success' => false,
						'message' => esc_html__( 'Data was not removed for id=',
								'wagency' ) . $_POST['post_id'],
					);
				}
			} else {
				$resp = array(
					'success' => false,
					'message' => esc_html__( 'You can\'t edit id=', 'wagency' ) . $_POST['post_id'],
				);
			}
		} else {
			$resp = array(
				'success' => false,
				'message' => esc_html__( 'Wrong data.', 'wagency' ),
			);
		}
