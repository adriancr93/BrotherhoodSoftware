		...
		if ( isset( $secretlab['header_widget'] ) && class_exists( 'custom_post_widget' ) ) {
			...
			if ( $atiframebuilder_layout[ $type . $prefix . 'header_widget' ] !== - 1 ) {
				...
				if ( ! empty( $styles['styles'] ) ) {
					...
					self::$header_widget = preg_replace( '@<(style)[^>]*?>.*?</\\1>@si', '', $header );
				} else {
	...
	public static function header_layout() {
		...
		if ( isset( $secretlab['header_widget'] ) && class_exists( 'custom_post_widget' ) ) {
            ...
            <div class="entry-meta">';
							...
							edit_post_link( esc_html__( 'Edit', 'wagency' ),
						...
						if ( is_search() ) {
							...
							printf( esc_html__( 'Search Results for: %s', 'wagency' ), get_search_query() );
							echo '</h1>';
							echo '</header>';
						}
						if ( is_day() ) {
							echo '<header class="custblog">';
							echo '<h1 class="archive-title">';
							printf( esc_html__( 'Daily Archives: %s', 'wagency' ), get_the_date() );
							echo '</h1>';
							echo '</header>';
						}
						if ( is_month() ) {
							echo '<header class="custblog">';
							echo '<h1 class="archive-title">';
							printf( esc_html__( 'Monthly Archives: %s', 'wagency' ),
								get_the_date( _x( 'F Y', 'monthly archives date format', 'wagency' ) ) );
							echo '</h1>';
							echo '</header>';
						}
						if ( is_year() ) {
							echo '<header class="custblog">';
							echo '<h1 class="archive-title">';
							printf( esc_html__( 'Yearly Archives: %s', 'wagency' ),
								get_the_date( _x( 'Y', 'yearly archives date format', 'wagency' ) ) );
							echo '</h1>';
							echo '</header>';
						}
						if ( is_404() ) {
							echo '<header class="custblog">
                       <h1>', esc_html__( '404', 'wagency' ), '</h1>
                    </header>
                    </div>';
						}
