...
} elseif( $show_svg == 'img' && '' !== $svg ) {
	if ( 'custom_size' == $img_size && '' !== $custom_img_size ) {
		$size      = ssc_get_img_sizes_array_from_string( $custom_img_size );
		$img_thumb = image_downsize( $svg, $size );
	} else {
		$img_thumb = image_downsize( $svg, $img_size );
	}
	$alt = get_post_meta( $svg, '_wp_attachment_image_alt', true);
	if( empty( $alt ) ){
		$alt = $title;
    }
	$data_icon_header .= '<div class="content-img-header">' . '<img src="' . $img_thumb['0'] . '" alt="' . $alt . '" />' . '</div>';
} else {
