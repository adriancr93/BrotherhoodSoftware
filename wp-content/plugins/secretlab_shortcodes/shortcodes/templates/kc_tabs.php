...
$title                  = $interval = $open_mouseover = $active_section = $effect_option = $class = $navigation = $auto_height = $tab_nav_class = $title_slider = $type = $vertical_tabs_position = '';
...
} else if ( $type == 'switcher_tabs' ) {
	$tab_nav_class = 'kc_tabs_nav switcher-nav';

	$tabs_option_data[] = 'class="template-' . $vertical_tabs_template . ' ' . esc_attr( implode( ' ', $css_class ) ) . '"';

	?>
    <div <?php echo implode( ' ', $tabs_option_data ); ?>>
        <div class="kc_wrapper ui-tabs switcher-tabs kc_clearfix">
            <div class="<?php echo esc_attr( $tab_nav_class ); ?> ui-tabs-nav kc_clearfix">
				<?php preg_replace_callback( '/kc_tab\s([^\]\#]+)/i', 'ssc_process_tab_switch_title', $content ); ?>
            </div>
