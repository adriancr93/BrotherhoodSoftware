...
stickerout = '',
    ...
    custom_img_size = ( atts['custom_img_size'] !== undefined )? atts['custom_img_size'] : '',
...
if ( show_icon_header === 'yes' ) {
	...
	} else if ( show_svg == 'img' && '' !== svg ) {
        var img;
        if( 'custom_size' == img_size ){
        if( custom_img_size.indexOf('x') > 0 ){
        img = ( ajaxurl + '?action=kc_get_thumbn_size&id=' + svg + '&size=' + custom_img_size );
        }else{
        img = ( ajaxurl + '?action=kc_get_thumbn&id=' + svg + '&size=' + custom_img_size );
        }
        } else {
        img = ( ajaxurl + '?action=kc_get_thumbn&id=' + svg + '&size=' + img_size);
        }
		data_icon_header += '<div class="content-img-header">';
		data_icon_header += '<img src="' + img + '" />';
