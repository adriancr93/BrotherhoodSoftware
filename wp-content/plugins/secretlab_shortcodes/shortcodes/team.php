...
if ( !empty( $title ) ) {
	...
	if( empty( $title_link[0] ) ){
		$data_title .= '<div class="content-title">'.$title.'</div>';
	} else {
		$target = '';
		if( !empty( $title_link[2] ) ){
			$target = 'target="' . $title_link[2] . '"';
		}

		$data_title .= '<div class="content-title"><a href="' . $title_link[0] . '" ' . $target . '>' . $title . '</a></div>';
	}
}
