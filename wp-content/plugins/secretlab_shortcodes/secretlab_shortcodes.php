...
function ssc_process_tab_switch_title( $matches ) {

	if ( ! empty( $matches[0] ) ) {

		$tab_class = 'ssc-switch-tab';
		...
		if ( isset( $tab_atts['advanced'] ) && $tab_atts['advanced'] === 'yes' ) {
			...
			echo '<div class="' . $tab_class . '">' . $adv_title . '</div>';
			...
			echo '<div class="' . $tab_class . '"><a href="#' . ( isset( $tab_atts['tab_id'] ) ? $tab_atts['tab_id'] : '' ) . '" data-prevent="scroll">' . $title . '</a></div>';
		}

		echo '<div class="ssc-switch-btn"></div>';
